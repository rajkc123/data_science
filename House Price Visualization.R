...
#creating box plot to visualize average house prices in kent and surrey in 2020
Grouped_houseprice %>% 
  filter(`Date of Transfer`==2020) %>% #filtering to show only houseprice data of 2020
  ...
  labs(title="2020 Average House Prices By County Box Plot") #setting label for the chart


#-----------2020 Average House Price Bar Chart-----------#

#creating bar chart to visualize average house prices in kent and surrey
Grouped_houseprice %>% 
  filter(`Date of Transfer`==2020) %>% #filtering to show only houseprice data of 2022
  group_by(County) %>% #grouping by county since we are comparing counties only
  ...
  scale_y_continuous(limits=c(0,2000000), breaks = seq(0,2000000,300000))+ #setting limits and breaks
  labs(title = "2020 Average House Prices Barchart") 
